...
struct channel_control {
    ...
    [[nodiscard]] uint8_t terminal_volume(const terminal t) const noexcept
    {
        return (nr_50.value() >> (terminal_bit_offset(t) * 4u)) & 0x7u;
    }

    [[nodiscard]] bool channel_enabled_on_terminal(const uint8_t channel_no, const terminal t) const noexcept
    {
        return bit::test(nr_51, terminal_bit_offset(t) + channel_no - 1u);
    }
